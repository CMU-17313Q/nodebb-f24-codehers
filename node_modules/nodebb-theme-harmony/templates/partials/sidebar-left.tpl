...
document.getElementById('bug-feedback-form').addEventListener('submit', function(event) {
    ...
    fetch(`${config.relative_path}/api/submit-bug`, {
        method: 'POST',
        headers: {
        ...
        if (response.ok) {
            ...
            closeForm();
            updateBugArchive(); // Refresh the bug archive
        } else {
...
function updateBugArchive() {
    fetch(`${config.relative_path}/api/admin/bug-archive`, { credentials: 'include' })
