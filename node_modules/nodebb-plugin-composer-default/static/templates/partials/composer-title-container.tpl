<div class="title-container align-items-center gap-2 d-flex">
	...
	<!-- Shortened title input to make space for the checkbox -->
	<div data-component="composer/title" class="position-relative flex-1" style="min-width: 0;">
		{{{ if isTopicOrMain }}}
		<input class="title form-control h-100 rounded-1 shadow-none" type="text" placeholder="[[topic:composer.title-placeholder]]" value="{topicTitle}" style="width: calc(100% - 80px);"/>
		{{{ else }}}
	...
	<div class="{{{ if !template.compose }}}d-none d-md-flex{{{ else }}}d-flex{{{ end }}} action-bar gap-1 align-items-center">
		<!-- New 'Post Anonymously' checkbox next to hide button -->
		<div class="form-check me-3">
			<input class="form-check-input" type="checkbox" id="anonymousPostCheckbox">
			 <label class="form-check-label text-body fw-semibold btn-sm"  for="anonymousPostCheckbox">
